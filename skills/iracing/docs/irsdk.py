...
class IRSDKStruct:
    ...
    def property_value_str(cls, offset, var_type):
        struct_type = struct.Struct(var_type)
        return property(
            lambda self: self.get(offset, struct_type).strip(b"\x00").decode("latin-1")
        )
...
class VarBuffer(IRSDKStruct):
    ...
    def freeze(self):
        self._frozen_memory = self._shared_mem[
            self._buf_offset : self._buf_offset + self._buf_len
        ]
...
class IRSDK:
    ...
    def is_connected(self):
        if self._header:
            ...
            if (
                self.__workaround_connected_state == 0
                and self._header.status != StatusField.status_connected
            ):
                self.__workaround_connected_state = 1
            if self.__workaround_connected_state == 1 and (
                self["SessionNum"] is None or self.__test_file
            ):
                self.__workaround_connected_state = 2
            if (
                self.__workaround_connected_state == 2
                and self["SessionNum"] is not None
            ):
                self.__workaround_connected_state = 3
        return (
            self._header is not None
            and (self.__test_file or self._data_valid_event)
            and (
                self._header.status == StatusField.status_connected
                or self.__workaround_connected_state == 3
            )
        )
    ...
    def var_headers_names(self):
        if self.__var_headers_names is None:
            self.__var_headers_names = [
                var_header.name for var_header in self._var_headers
            ]
    ...
    def startup(self, test_file=None, dump_to=None):
        if test_file is None:
            if not self._check_sim_status():
                return False
            self._data_valid_event = ctypes.windll.kernel32.OpenEventW(
                0x00100000, False, DATAVALIDEVENTNAME
            )
        ...
        if self._shared_mem is None:
            if test_file:
                ...
                self._shared_mem = mmap.mmap(
                    self.__test_file.fileno(), 0, access=mmap.ACCESS_READ
                )
            else:
                self._shared_mem = mmap.mmap(
                    0, MEMMAPFILESIZE, MEMMAPFILE, access=mmap.ACCESS_READ
                )
        ...
        if self._shared_mem:
            ...
            self.is_initialized = (
                self._header.version >= 1 and len(self._header.var_buf) > 0
            )
    ...
    def parse_to(self, to_file):
        ...
        f.write(
            self._shared_mem[
                self._header.session_info_offset : self._header.session_info_len
            ]
            .rstrip(b"\x00")
            .decode(YAML_CODE_PAGE)
        )
        f.write(
            "\n".join(
                [
    ...
    def cam_switch_num(self, car_number="1", group=1, camera=0):
        return self._broadcast_msg(
            BroadcastMsg.cam_switch_num, self._pad_car_num(car_number), group, camera
        )
    ...
    def replay_set_play_speed(self, speed=0, slow_motion=False):
        return self._broadcast_msg(
            BroadcastMsg.replay_set_play_speed, speed, 1 if slow_motion else 0
        )

    def replay_set_play_position(self, pos_mode=RpyPosMode.begin, frame_num=0):
        return self._broadcast_msg(
            BroadcastMsg.replay_set_play_position, pos_mode, frame_num
        )
    ...
    def reload_texture(self, car_idx=0):
        return self._broadcast_msg(
            BroadcastMsg.reload_textures, ReloadTexturesMode.car_idx, car_idx
        )
    ...
    def chat_command_macro(self, macro_num=0):
        return self._broadcast_msg(
            BroadcastMsg.chat_command, ChatCommandMode.macro, macro_num
        )
    ...
    def ffb_command(
        self, ffb_command_mode=FFBCommandMode.ffb_command_max_force, value=0
    ):
        return self._broadcast_msg(
            BroadcastMsg.ffb_command, ffb_command_mode, int(value * 65536)
        )

    def replay_search_session_time(self, session_num=0, session_time_ms=0):
        return self._broadcast_msg(
            BroadcastMsg.replay_search_session_time, session_num, session_time_ms
        )
        ...
        if self.__var_headers is None:
            ...
            for i in range(self._header.num_vars):
                var_header = VarHeader(
                    self._shared_mem, self._header.var_header_offset + i * 144
                )
    ...
    def freeze_var_buffer_latest(self):
        ...
        self.__var_buffer_latest = sorted(
            self._header.var_buf, key=lambda v: v.tick_count, reverse=True
        )[0]
    ...
    def _wait_valid_data_event(self):
        if self._data_valid_event is not None:
            return (
                ctypes.windll.kernel32.WaitForSingleObject(self._data_valid_event, 32)
                == 0
    ...
    def _get_session_info(self, key):
        ...
        if self.parse_yaml_async:
            if (
                "async_session_info_update" not in session_data
                or session_data["async_session_info_update"]
                < self.last_session_info_update
            ):
                session_data["async_session_info_update"] = (
                    self.last_session_info_update
                )
    ...
    def _get_session_info_binary(self, key):
        ...
        match_start = re.compile(("\n%s:\n" % key).encode(YAML_CODE_PAGE)).search(
            self._shared_mem, start, end
        )
        if not match_start:
            return None
        match_end = re.compile(b"\n\n").search(
            self._shared_mem, match_start.start() + 1, end
        )
        if not match_end:
            return None
    ...
    def _parse_yaml(self, key, session_data):
        ...
        if (
            "data_binary" in session_data
            and data_binary == session_data["data_binary"]
            and "data_last" in session_data
        ):
        ...
        yaml_src = re.sub(
            ...
            "",
            data_binary.translate(YAML_TRANSLATER)
            .rstrip(b"\x00")
            .decode(YAML_CODE_PAGE),
        )
        if key == "DriverInfo":
        ...
        if result and (
            not self.parse_yaml_async
            or self.last_session_info_update == session_info_update
        ):
    ...
    def _broadcast_msg_id(self):
        if self.__broadcast_msg_id is None:
            self.__broadcast_msg_id = ctypes.windll.user32.RegisterWindowMessageW(
                BROADCASTMSGNAME
            )
    ...
    def _broadcast_msg(self, broadcast_type=0, var1=0, var2=0, var3=0):
        return ctypes.windll.user32.SendNotifyMessageW(
            0xFFFF,
            self._broadcast_msg_id,
            broadcast_type | var1 << 16,
            var2 | var3 << 16,
        )
...
class IBT:
    ...
    def open(self, ibt_file):
        ...
        self._shared_mem = mmap.mmap(
            self._ibt_file.fileno(), 0, access=mmap.ACCESS_READ
        )
        self._header = Header(self._shared_mem)
    ...
    def get(self, index, key):
        ...
        if key in self._var_headers_dict:
            ...
            var_offset = (
                var_header.offset
                + self._header.var_buf[0].buf_offset
                + index * self._header.buf_len
            )
    ...
    def get_all(self, key):
        ...
        if key in self._var_headers_dict:
            ...
            for i in range(self._disk_header.session_record_count):
                res = struct.unpack_from(
                    fmt, self._shared_mem, var_offset + i * buf_len
                )
...
def main():
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version="Python iRacing SDK %s" % VERSION,
