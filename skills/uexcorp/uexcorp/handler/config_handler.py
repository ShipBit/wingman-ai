...
class ConfigHandler:
    def __init__(self, helper: "Helper"):
        ...
        self.__wingman: "Wingman | None" = None
        self.__fine_config_path: str = get_writable_dir(
            os.path.join(self.__helper.get_data_path(), "config")
        )
    ...
    async def validate(
        ...
        retrieve_custom_property_value: callable,
    ) -> list[WingmanInitializationError]:
        ...
        try:
            ...
            self.set_behavior_commodity_route_use_estimated_availability(
                retrieve_custom_property_value(
                    "commodity_route_use_estimated_availability", errors
                )
            )
            ...
            if self.__behavior_enabled_tools_need_authentication:
                api_key = await self.__helper.get_handler_secret().retrieve(
                    requester="UEX config service", key="uex", prompt_if_missing=True
                )
                if api_key:
            ...
            self.__helper.get_handler_debug().write(
                f"Error while validating config: {e}", True
            )
            self.__helper.get_handler_error().write(
                "ConfigHandler.validate", [errors], e
            )
            errors.append(
    ...
    def __sync_commodity_blacklist(self):
        ...
        file_path = os.path.join(self.__fine_config_path, "commodity_blacklist.yaml")
            ...
            try:
                ...
                if commodity_data:
                    for index, commodity in enumerate(commodity_data):
                        commodity_model = CommodityDataAccess().load_by_property(
                            "id", commodity["id"]
                        )
                        if commodity_model is None:
                            continue
                        commodity_model.set_is_blacklisted(
                            bool(commodity["is_blacklisted"])
                        )
                        commodity_model.persist(index < len(commodity_data) - 1)
            except Exception as e:
                self.__helper.get_handler_debug().write(
                    f"Error while syncing commodity blacklist: {e}", True
                )
                self.__helper.get_handler_error().write(
                    "ConfigHandler.__init_commodity_blacklist", [], e
                )
                self.__helper.get_handler_debug().write(
                    "Commodity blacklist config will be recreated.", True
                )
        ...
        with open(file_path, "w") as file:
            file.write(
                "# Only the 'is_blacklisted' value must be changed to 'true' or 'false'."
            )
            file.write(
    ...
    def __sync_terminal_blacklist(self):
        ...
        file_path = os.path.join(self.__fine_config_path, "terminal_blacklist.yaml")
        ...
        if os.path.exists(file_path):
            try:
                ...
                for index, terminal in enumerate(terminal_data):
                    terminal_model = TerminalDataAccess().load_by_property(
                        "id", terminal["id"]
                    )
                ...
                self.__helper.get_handler_debug().write(
                    f"Error while syncing terminal blacklist: {e}", True
                )
                self.__helper.get_handler_error().write(
                    "ConfigHandler.__init_terminal_blacklist", [], e
                )
                self.__helper.get_handler_debug().write(
                    "Terminal blacklist config will be recreated.", True
                )
        ...
        try:
            ...
            self.__helper.get_handler_debug().write(
                f"Error while writing terminal blacklist: {e}", True
            )
            self.__helper.get_handler_error().write(
                "ConfigHandler.__init_terminal_blacklist", [], e
            )
    ...
    def get_behavior_commodity_route_use_estimated_availability(self) -> bool:
        errors = []
        value = self.__property_retriever(
            "commodity_route_use_estimated_availability", errors
        )
        if not errors:
    ...
    def set_behavior_commodity_route_use_estimated_availability(
        self, use_estimated_availability: bool
    ):
        self.__behavior_commodity_route_use_estimated_availability = (
    ...
    def set_wingman(self, wingman: "Wingman"):
        self.__wingman = wingman

    def get_wingman(self) -> "Wingman":
