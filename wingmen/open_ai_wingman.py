...
class OpenAiWingman(Wingman):
    ...
    async def _condense_history(self, force: bool = False):
        ...
        try:
            ...
            corrected_available = int(
                available_tokens / self._summarize_token_ratio
            )
            ...
            if summarize_result and summarize_result.prompt_tokens > 0:
                cl100k_input = system_prompt_tokens + count_tokens(
                    condensed_text
                ) + prefix_suffix_tokens
                if cl100k_input > 0:
                    self._summarize_token_ratio = (
    ...
    async def _chunked_summarize(
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Give each chunk a generous output budget (apply ratio to input estimate)
            ...
            if result.text:
                ...
                # Calibrate tokenizer ratio from chunk usage
                if result.prompt_tokens > 0:
                    cl100k_input = chunk_input_tokens
                    if cl100k_input > 0:
                        self._summarize_token_ratio = (
        ...
        merge_input_tokens = int(
            (count_tokens(system_prompt) + count_tokens(merge_prompt))
            * self._summarize_token_ratio
        )
        merge_output_budget = max(512, n_ctx - merge_input_tokens)
